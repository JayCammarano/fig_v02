import React from "react";

const ArtistPlaceholderTile = ({ launchLogin, launchModal, loggedInStatus }) => {
  const onClickModal = () => {
    if (loggedInStatus === "LOGGED_IN") {
      launchModal("is-active");
    } else {
      launchLogin("is-active")
    }
  };

  return (
    <div className="card has-background-light" onClick={onClickModal}>
      <figure className="image is-48by48">
        <img
          src="data:image/png;base64,"
          className="card-image"
          alt="Artist Image"
        />
      </figure>
      <h4 className="card-header-title has-text-dark">Add a New Artist</h4>
    </div>
  );
};

export default ArtistPlaceholderTile;
